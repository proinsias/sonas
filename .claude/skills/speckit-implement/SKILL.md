...
- If it exists, read it and look for entries under the `hooks.before_implement`
  key
...
4. **Project Setup Verification**:
    ...
    **Detection & Creation Logic**:
    - Check if the following command succeeds to determine if the repository is
      a git repo (create/verify .gitignore if so):
    ...
    **Common Patterns by Technology** (from plan.md tech stack):
    ...
    - **C**: `build/`, `bin/`, `obj/`, `out/`, `*.o`, `*.a`, `*.so`, `*.exe`,
      `*.dll`, `autom4te.cache/`, `config.status`, `config.log`, `.idea/`,
      `*.log`, `.env*`
...
1. **Check for extension hooks**: After completion validation, check if
