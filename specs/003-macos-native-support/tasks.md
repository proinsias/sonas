...
- [ ] T001 Create `MacSonas/` directory with placeholder `Assets.xcassets` (all required macOS icon sizes, placeholder
      image) parallel to `WatchSonas/` and `TVSonas/`
- [ ] T002 Create `MacSonas/Info.plist` with `CFBundleDisplayName`, `NSLocationWhenInUseUsageDescription`,
      `NSPhotoLibraryUsageDescription`, `GIDClientID`, `LSApplicationCategoryType`, `LSUIElement: false`
- [ ] T003 [P] Update `project.yml` — add `MacSonas` target (platform: macOS, deploymentTarget: 15.0, sources:
...
- [ ] T004 [P] Update `project.yml` — set `SUPPORTS_MACCATALYST: NO` in `targets.Sonas.settings.base` (removes Catalyst
      build from iOS target per FR-001 clarification)
- [ ] T005 Create stub files in `Sonas/Platform/macOS/`: `MacSonasApp.swift`, `MacShell.swift`, `MacSidebarView.swift`,
...
- [ ] T006 Run `xcodegen generate`, open Xcode, confirm `MacSonas` scheme builds without errors and `Sonas` (iOS) scheme
      still builds without errors
- [ ] T007 [P] Create `SonasTests/MacNotificationServiceTests.swift` with 5 failing contract test stubs (one per test in
...
- [ ] T008 [P] Create `MacSonasUITests/` directory and target in `project.yml` with skeleton test files:
...
- [ ] T009 Add `.sonasWindowOpenRequested` `Notification.Name` extension to
      `Sonas/Shared/Extensions/Notification+Sonas.swift` (used by `MacNotificationService` delegate to trigger window
      open + navigation)
- [ ] T010 Implement `MacSonasApp.swift` (`Sonas/Platform/macOS/MacSonasApp.swift`) as the `@main` entry point with: a
...
- [ ] T011 [US1] Implement `MacSidebarView.swift` (`Sonas/Platform/macOS/MacSidebarView.swift`):
...
- [ ] T012 [US1] Implement `MacShell.swift` (`Sonas/Platform/macOS/MacShell.swift`): `NavigationSplitView` with
...
- [ ] T013 [US1] Wire all `AppSection` cases in `MacShell.detailView(for:)` to their feature panel views (same mapping
      as `IPadShell.detailView(for:)` in `Sonas/Platform/iPad/IPadShell.swift`) in `Sonas/Platform/macOS/MacShell.swift`
- [ ] T014 [US1] Add macOS toolbar to `MacShell` (`Sonas/Platform/macOS/MacShell.swift`): `.toolbar` modifier with a
      Refresh `ToolbarItem` that posts `.sonasRefreshRequested`; set `.navigationTitle` per selected section
- [ ] T015 [US1] Update `MacSonasApp.swift` (`Sonas/Platform/macOS/MacSonasApp.swift`) `WindowGroup` body to use
      `MacShell()`; add `.defaultSize(width: 1200, height: 800)`
- [ ] T016 [US1] Write `MacDashboardUITests.swift` (`MacSonasUITests/MacDashboardUITests.swift`): implement acceptance
...
- [ ] T017 [US2] Implement `MenuBarState.swift` (`Sonas/Platform/macOS/MenuBarState.swift`):
...
- [ ] T018 [US2] Implement `MacMenuBarPopoverView.swift` (`Sonas/Platform/macOS/MacMenuBarPopoverView.swift`): compact
...
- [ ] T019 [US2] Wire `MenuBarExtra` in `MacSonasApp.swift` (`Sonas/Platform/macOS/MacSonasApp.swift`):
...
- [ ] T020 [US2] Create `MenuBarState` instance in `MacSonasApp` and inject via `.environment(menuBarState)` on both
...
- [ ] T021 [US2] Write `MacMenuBarUITests.swift` (`MacSonasUITests/MacMenuBarUITests.swift`): acceptance tests for US2
...
- [ ] T022 [US3] Implement `MacSonasCommands.swift` (`Sonas/Platform/macOS/MacSonasCommands.swift`):
      `CommandGroup(replacing: .newItem)` with "New Window" button calling
      `NSApp.sendAction(Selector(("newWindowForTab:")), to: nil, from: nil)` or `@Environment(\.openWindow)` approach;
      `CommandGroup(replacing: .appSettings)` with "Settings…" (Cmd+,) posting `.sonasSettingsRequested`;
      `CommandGroup(after: .pasteboard)` with "Refresh All" (Cmd+R) posting `.sonasRefreshRequested`
- [ ] T023 [US3] Add `CommandMenu("View")` to `MacSonasCommands` (`Sonas/Platform/macOS/MacSonasCommands.swift`): one
...
- [ ] T024 [US3] Wire `.commands { MacSonasCommands() }` on the `WindowGroup` scene in `MacSonasApp.swift`
      (`Sonas/Platform/macOS/MacSonasApp.swift`), replacing the stub
- [ ] T025 [US3] Write `MacKeyboardShortcutUITests.swift` (`MacSonasUITests/MacKeyboardShortcutUITests.swift`):
...
- [ ] T026 [US4] Implement `MacNotificationService.swift` (`Sonas/Platform/macOS/MacNotificationService.swift`):
...
- [ ] T027 [US4] Implement `UNUserNotificationCenterDelegate` conformance in `MacNotificationService`
...
- [ ] T028 [US4] Call `Task { await MacNotificationService.shared.register() }` in `MacSonasApp.init()`
...
- [ ] T029 [US4] Extend `LocationViewModel` with `#if os(macOS)` block
...
- [ ] T030 [US4] Extend `EventsViewModel` (or `DashboardViewModel`) with `#if os(macOS)` block: after loading events,
...
- [ ] T031 [US4] Complete all 5 failing stubs in `SonasTests/MacNotificationServiceTests.swift` so they pass: inject a
...
- [ ] T032 [US4] Write `MacNotificationUITests.swift` (`MacSonasUITests/MacNotificationUITests.swift`): acceptance tests
...
- [ ] T033 [US5] Add `.windowResizability(.contentSize)` and `defaultSize(width: 1200, height: 800)` to the
...
- [ ] T034 [US5] Verify `@SceneStorage("mac.selectedSection")` is independent per window: open two windows, navigate
...
- [ ] T035 [US5] Write `MacMultiWindowUITests.swift` (`MacSonasUITests/MacMultiWindowUITests.swift`): acceptance tests
...
- [ ] T036 Implement network reachability check in `MenuBarState.swift` (`Sonas/Platform/macOS/MenuBarState.swift`): use
...
- [ ] T037 Add offline banner to `MacShell` detail area (`Sonas/Platform/macOS/MacShell.swift`): when
...
- [ ] T038 Show "Last updated [relative time]" footer in `MacMenuBarPopoverView`
      (`Sonas/Platform/macOS/MacMenuBarPopoverView.swift`) when `menuBarState.isOffline` is true, below the weather
      section
- [ ] T039 Manual validation: disable Wi-Fi, launch `MacSonas`, verify all sections display cached data within 1s
...
- [ ] T040 [P] Replace placeholder app icon in `MacSonas/Assets.xcassets` with final production macOS app icon (provide
      all required sizes: 16, 32, 64, 128, 256, 512, 1024pt)
- [ ] T041 [P] Verify SC-006: run a full clean build of `MacSonas` scheme and confirm zero Catalyst-era warnings in the
      Xcode build log; confirm `SUPPORTS_MACCATALYST: NO` is effective on the `Sonas` iOS scheme
- [ ] T042 [P] Run Instruments Allocations on `MacSonas`: open and close 5 windows; verify no per-window retain cycles
      or growing heap; document baseline memory reading in this task's completion note
- [ ] T043 Add `MacSonas` scheme to GitHub Actions CI (`.github/workflows/ci.yml` or equivalent): build + test step
      targeting `platform=macOS`
- [ ] T044 Prepare Mac App Store submission in App Store Connect: create new macOS app record (separate from iOS
