...
**Purpose**: Access the designated iCloud Shared Album via PhotoKit; provide photo metadata
and on-demand image loading.
...
**Error cases**:
- `PhotoServiceError.permissionDenied` — photo library access denied; panel shows "Enable
  photo access in Settings" prompt.
- `PhotoServiceError.noAlbumSelected` — no shared album configured; panel shows "Select a
  shared album" prompt.
- `PhotoServiceError.albumEmpty` — selected album has no photos; panel shows "Add photos to
  your shared album" prompt.
