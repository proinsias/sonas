...
- No build-time configuration. Users enter their personal API token at runtime in
  Settings → Connect Todoist.
...
Coverage gate: `xcodebuild test ... -enableCodeCoverage YES` — CI fails if `Sonas/` coverage
drops below 80%.
...
- Select an iPad simulator (e.g., iPad Pro 13-inch) → the dashboard switches to the 3-column
  grid layout automatically via `horizontalSizeClass == .regular`.
- For Mac (designed for iPad): Product → Destination → My Mac (Designed for iPad).
  Mouse/keyboard navigation should reach all controls.

---

## 8. CloudKit schema setup (first run)

CloudKit schema is created automatically on first app launch (development containers only).
For production, export the schema via CloudKit Dashboard and promote to production before
