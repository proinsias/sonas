...
struct WeatherContractTests {
    ...
    @Test("given AQI stub returns 42 when weather fetched then snapshot airQualityIndex is 42")
    func given_aqiStub42_when_weatherFetched_then_airQualityIndex42() async throws {
    ...
    @Test("given mock weather service when fetchWeather called then forecast contains 7 days")
    func given_mockService_when_fetchWeather_then_forecastCount7() async throws {
        let mock = WeatherServiceMock()
    ...
    @Test("given mock weather service when fetchWeather called then forecast[0].id equals today's date")
    func given_mockService_when_fetchWeather_then_firstForecastIdIsToday() async throws {
        let mock = WeatherServiceMock()
        let (_, forecast) = try await mock.fetchWeather(
            for: CLLocationCoordinate2D(latitude: 53.35, longitude: -6.26)
        )
    ...
    @Test("given mock weather service when fetchWeather called then all 8 required attributes are populated")
    func given_mockService_when_fetchWeather_then_all8AttributesPopulated() async throws {
        let mock = WeatherServiceMock()
        let (snapshot, _) = try await mock.fetchWeather(
            for: CLLocationCoordinate2D(latitude: 53.35, longitude: -6.26)
        )
