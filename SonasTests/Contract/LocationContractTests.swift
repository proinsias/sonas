...
struct LocationContractTests {
    ...
    @Test
    func `given two CloudKit FamilyLocation records when refresh is called then familyLocations_emits_2_members`(
    ) async throws {
    ...
    @Test
    func `given a CloudKit record with placeName when parsed then FamilyMember_location_placeName_matches`() {
    ...
    @Test
    func `given a CloudKit record with recordedAt when parsed then FamilyMember_location_recordedAt_is_recent`() {
        let now = Date.now
        let record = Self.makeFamilyLocationRecord(
