...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "       [--short-name <name>] [--number N] [--timestamp] <feature_description>"
            echo ""
            echo "Options:"
...
if [ -z "$FEATURE_DESCRIPTION" ]; then
    echo "Usage: $0 [--json] [--dry-run] [--allow-existing-branch]" >&2
    echo "       [--short-name <name>] [--number N] [--timestamp] <feature_description>" >&2
    exit 1
fi
...
generate_branch_name() {
    ...
    local stop_words
    stop_words="^(i|a|an|the|to|for|of|in|on|at|by|with|from|is|are|was|were"
    stop_words="$stop_words|been|being|have|has|had|do|does|did|will|would|should"
    stop_words="$stop_words|could|can|may|might|must|shall|this|that|these|those"
    stop_words="$stop_words|my|your|our|their|want|need|add|get|set)$"
            ...
            if git branch --list "$BRANCH_NAME" | grep -q .; then
                if [ "$ALLOW_EXISTING" = true ]; then
                    if [ "$current_branch" = "$BRANCH_NAME" ]; then
                        ...
                        >&2 echo "Please resolve any local changes or conflicts and try again."
                    ...
                    >&2 echo "Rerun to get a new timestamp or use a different --short-name."
                    exit 1
                else
                    >&2 echo "Error: Branch '$BRANCH_NAME' already exists."
                    >&2 echo "Please use a different feature name or specify a different number with --number."
                    exit 1
                fi
            else
